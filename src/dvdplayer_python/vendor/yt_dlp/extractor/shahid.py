...
class ShahidBaseIE(AWSIE):
    ...
    # Redacted in this vendored standalone build (YouTube-only usage).
    _AWS_API_KEY = 'REDACTED_FOR_STANDALONE_BUILD'
    ...
    def _call_api(self, path, video_id, request=None):
        ...
        try:
            return self._aws_execute_api({
                'uri': '/proxy/v2/' + path,
                'access_key': 'REDACTED_FOR_STANDALONE_BUILD',
                'secret_key': 'REDACTED_FOR_STANDALONE_BUILD',
            }, video_id, query)
